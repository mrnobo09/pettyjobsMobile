...
const ViewJobScreen = () => {
  ...
  const handleJobApproval = (approved) => {
    ...
    axios.post(`${backendURL}/app/accept_or_reject_reviewed_job_as_worker/`, body, config)
        .then(response => {
    ...
    <View style={styles.mainContainer}>
      ...
      {jobDetails.status === 'review' && (
        ...
        <Text>Satisfied By The Job?</Text>
          <TouchableOpacity style={[styles.bottomBoxButton,{backgroundColor:'#0e2064'}]} onPress={()=>handleJobApproval(true)}><Text style={{color:'#ffffff'}}>Yes</Text></TouchableOpacity>
          <TouchableOpacity style={[styles.bottomBoxButton,{backgroundColor:'#b70b0b'}]} onPress={()=>handleJobApproval(false)}><Text style={{color:'#ffffff'}}>No</Text></TouchableOpacity>
        </View>
      )}
    </View>
  );
